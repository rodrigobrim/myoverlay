...
S: dict[str, dict[str, str]] = {
    ...
    "DEPB_EXPLAIN": dict(zip(L, [
        "MyOverlay cannot work without it. Install it now (keep this window open), then click Back and Next to check again - or click Cancel to exit setup and run it again later.",
        "O MyOverlay não funciona sem ele. Instale-o agora (mantenha esta janela aberta) e depois clique em Voltar e Avançar para verificar novamente - ou clique em Cancelar para sair da instalação e executá-la de novo mais tarde.",
        "MyOverlay no puede funcionar sin él. Instálelo ahora (mantenga esta ventana abierta) y luego haga clic en Atrás y Siguiente para comprobar de nuevo - o haga clic en Cancelar para salir de la instalación y ejecutarla de nuevo más tarde.",
        "MyOverlay はこれなしでは動作しません。今すぐインストールし（このウィンドウは開いたままにしてください）、[戻る]と[次へ]をクリックして再確認してください。または[キャンセル]をクリックしてセットアップを終了し、後で再実行してください。",
        "لا يمكن لـ MyOverlay العمل بدونه. قم بتثبيته الآن (أبق هذه النافذة مفتوحة)، ثم انقر فوق السابق ثم التالي لإعادة التحقق - أو انقر فوق إلغاء للخروج من الإعداد وتشغيله مرة أخرى لاحقا.",
        "MyOverlay ne peut pas fonctionner sans lui. Installez-le maintenant (gardez cette fenêtre ouverte), puis cliquez sur Précédent puis Suivant pour vérifier à nouveau - ou cliquez sur Annuler pour quitter l'installation et la relancer plus tard.",
        "MyOverlay non può funzionare senza. Installalo ora (tieni aperta questa finestra), poi fai clic su Indietro e Avanti per verificare di nuovo - oppure fai clic su Annulla per uscire dall'installazione ed eseguirla di nuovo più tardi.",
        "MyOverlay не может работать без неё. Установите её сейчас (не закрывайте это окно), затем нажмите «Назад» и «Далее», чтобы проверить снова, - или нажмите «Отмена», чтобы выйти из установки и запустить её позже.",
    ])),
    ...
    "GOOGLE_FAIL": dict(zip(L, [
        "Google setup did not finish, so YouTube upload is not configured yet. The application itself installed fine. Run 'MyOverlay google-setup' to try again. Details:",
        "A configuração do Google não foi concluída, então o envio para o YouTube ainda não está configurado. O aplicativo em si foi instalado corretamente. Execute 'MyOverlay google-setup' para tentar novamente. Detalhes:",
        "La configuración de Google no se completó, así que la subida a YouTube aún no está configurada. La aplicación se instaló correctamente. Ejecute 'MyOverlay google-setup' para intentarlo de nuevo. Detalles:",
        "Google の設定が完了しなかったため、YouTube へのアップロードはまだ設定されていません。アプリケーション自体は正常にインストールされました。「MyOverlay google-setup」を実行して再試行してください。詳細:",
        "لم يكتمل إعداد Google، لذا لم يتم تكوين الرفع إلى YouTube بعد. تم تثبيت التطبيق نفسه بنجاح. شغّل 'MyOverlay google-setup' للمحاولة مرة أخرى. التفاصيل:",
        "La configuration Google ne s'est pas terminée, l'envoi vers YouTube n'est donc pas encore configuré. L'application elle-même est bien installée. Exécutez 'MyOverlay google-setup' pour réessayer. Détails :",
        "La configurazione di Google non è stata completata, quindi il caricamento su YouTube non è ancora configurato. L'applicazione è stata installata correttamente. Esegui 'MyOverlay google-setup' per riprovare. Dettagli:",
        "Настройка Google не была завершена, поэтому загрузка на YouTube ещё не настроена. Само приложение установлено успешно. Запустите 'MyOverlay google-setup', чтобы повторить попытку. Подробности:",
    ])),
    ...
    "RO_EXPLAIN": dict(zip(L, [
        "Uninstalling removes the MyOverlay application and its shortcuts. Everything else stays on this computer, ready for a future install - tick a box below only for what you also want deleted.",
        "A desinstalação remove o aplicativo MyOverlay e seus atalhos. Todo o resto permanece neste computador, pronto para uma instalação futura - marque uma opção abaixo apenas para o que você também quiser excluir.",
        "La desinstalación elimina la aplicación MyOverlay y sus accesos directos. Todo lo demás permanece en este equipo, listo para una futura instalación: marque una casilla solo para lo que también quiera eliminar.",
        "アンインストールでは MyOverlay アプリケーションとそのショートカットを削除します。それ以外はこのコンピューターに残り、次回のインストールでそのまま使えます。併せて削除したいものだけ、下のチェックボックスをオンにしてください。",
        "تؤدي إزالة التثبيت إلى إزالة تطبيق MyOverlay واختصاراته. يبقى كل ما عداه على هذا الكمبيوتر، جاهزًا لتثبيت لاحق - حدد خانة أدناه فقط لما تريد حذفه أيضًا.",
        "La désinstallation supprime l'application MyOverlay et ses raccourcis. Tout le reste demeure sur cet ordinateur, prêt pour une installation ultérieure : cochez une case ci-dessous uniquement pour ce que vous voulez aussi supprimer.",
        "La disinstallazione rimuove l'applicazione MyOverlay e i suoi collegamenti. Tutto il resto resta su questo computer, pronto per un'installazione futura: seleziona una casella qui sotto solo per ciò che vuoi eliminare.",
        "Удаление убирает приложение MyOverlay и его ярлыки. Всё остальное остаётся на этом компьютере и готово к будущей установке - отметьте пункт ниже только для того, что нужно удалить тоже.",
    ])),
    "RO_REPO": dict(zip(L, [
        "Also delete the local copy of the repository",
        "Excluir também a cópia local do repositório",
        "Eliminar también la copia local del repositorio",
        "リポジトリのローカル コピーも削除する",
        "حذف النسخة المحلية من المستودع أيضًا",
        "Supprimer aussi la copie locale du dépôt",
        "Elimina anche la copia locale del repository",
        "Также удалить локальную копию репозитория",
    ])),
    "RO_REPO_NOTE": dict(zip(L, [
        "It is downloaded again on the next install; delete it to also discard local branches or edits.",
        "Ela é baixada novamente na próxima instalação; exclua-a para descartar também branches ou edições locais.",
        "Se descarga de nuevo en la próxima instalación; elimínela para descartar también ramas o ediciones locales.",
        "次回のインストール時に再ダウンロードされます。ローカルのブランチや編集も破棄する場合に削除してください。",
        "يتم تنزيلها مرة أخرى عند التثبيت التالي؛ احذفها للتخلص أيضًا من الفروع أو التعديلات المحلية.",
        "Elle est retéléchargée à la prochaine installation ; supprimez-la pour écarter aussi les branches ou modifications locales.",
        "Viene scaricata di nuovo alla prossima installazione; eliminala per scartare anche branch o modifiche locali.",
        "Она загрузится заново при следующей установке; удалите, чтобы отбросить и локальные ветки или правки.",
    ])),
    "RO_GOOGLE": dict(zip(L, [
        "Also delete the Google sign-in and credentials",
        "Excluir também o login do Google e as credenciais",
        "Eliminar también el inicio de sesión de Google y las credenciales",
        "Google のサインインと認証情報も削除する",
        "حذف تسجيل دخول Google وبيانات الاعتماد أيضًا",
        "Supprimer aussi la connexion Google et les identifiants",
        "Elimina anche l'accesso a Google e le credenziali",
        "Также удалить вход в Google и учётные данные",
    ])),
    "RO_GOOGLE_NOTE": dict(zip(L, [
        "The OAuth client secret cannot be downloaded again: a future install has to create a new client and authorize YouTube from scratch.",
        "O secret do cliente OAuth não pode ser baixado de novo: uma instalação futura terá de criar um novo cliente e autorizar o YouTube do zero.",
        "El secreto de cliente OAuth no se puede volver a descargar: una futura instalación tendrá que crear un cliente nuevo y autorizar YouTube desde cero.",
        "OAuth クライアント シークレットは再ダウンロードできません。次回のインストールでは新しいクライアントを作成し、YouTube の認可をやり直す必要があります。",
        "لا يمكن تنزيل سر عميل OAuth مرة أخرى: سيتعين على أي تثبيت لاحق إنشاء عميل جديد وتفويض YouTube من البداية.",
        "Le secret client OAuth ne peut pas être retéléchargé : une installation ultérieure devra créer un nouveau client et réautoriser YouTube.",
        "Il client secret OAuth non è più scaricabile: un'installazione futura dovrà creare un nuovo client e autorizzare YouTube da zero.",
        "Секрет клиента OAuth нельзя скачать повторно: будущей установке придётся создать новый клиент и заново авторизовать YouTube.",
    ])),
...
ZH: dict[str, str] = {
    ...
    "GOOGLE_FAIL": "Google 设置未完成，因此尚未配置 YouTube 上传。应用程序本身已正确安装。请运行“MyOverlay google-setup”重试。详情：",
    ...
    "RO_TITLE": "卸载选项",
    "RO_DESC": "选择要删除的内容。",
    "RO_EXPLAIN": "卸载会删除 MyOverlay 应用程序及其快捷方式。其余内容都会保留在此计算机上，可供以后安装时使用 - 仅当您也想删除某项时，才勾选下面的复选框。",
    "RO_REPO": "同时删除仓库的本地副本",
    "RO_REPO_NOTE": "下次安装时会重新下载；如需一并丢弃本地分支或修改，请删除它。",
    "RO_GOOGLE": "同时删除 Google 登录信息和凭据",
    "RO_GOOGLE_NOTE": "OAuth 客户端密钥无法再次下载：以后安装时必须新建客户端并重新授权 YouTube。",
