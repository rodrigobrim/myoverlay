...
S: dict[str, dict[str, str]] = {
    ...
    "GA_NEXT": dict(zip(L, [
        "The first time you run MyOverlay, sign in once with the Google account that owns your YouTube channel. That is the only step you need to take.",
        "Na primeira vez que abrir o MyOverlay, entre uma vez com a conta Google dona do seu canal do YouTube. Esse é o único passo que você precisa dar.",
        "La primera vez que abra MyOverlay, inicie sesión una vez con la cuenta de Google propietaria de su canal de YouTube. Ese es el único paso que debe dar.",
        "MyOverlay を初めて起動したら、YouTube チャンネルを所有する Google アカウントで 1 回サインインしてください。必要な操作はこれだけです。",
        "عند تشغيل MyOverlay لأول مرة، سجّل الدخول مرة واحدة بحساب Google المالك لقناتك على YouTube. هذه هي الخطوة الوحيدة المطلوبة منك.",
        "Au premier lancement de MyOverlay, connectez-vous une fois avec le compte Google propriétaire de votre chaîne YouTube. C'est la seule étape à effectuer.",
        "Al primo avvio di MyOverlay, accedi una volta con l'account Google proprietario del tuo canale YouTube. È l'unico passaggio che devi fare.",
        "При первом запуске MyOverlay один раз войдите в аккаунт Google, которому принадлежит ваш канал YouTube. Это единственное, что нужно сделать.",
    ])),
    ...
    "SW_TEXT": dict(zip(L, [
        "Without it, MyOverlay will not upload videos to YouTube. Everything else works normally, and you can set this up later from MyOverlay whenever you want.",
        "Sem isso, o MyOverlay não enviará vídeos ao YouTube. Todo o resto funciona normalmente, e você pode configurar isso depois no MyOverlay quando quiser.",
        "Sin esto, MyOverlay no subirá vídeos a YouTube. Todo lo demás funciona con normalidad, y puede configurarlo más tarde desde MyOverlay cuando quiera.",
        "設定しない場合、MyOverlay は YouTube に動画をアップロードしません。それ以外は通常どおり動作し、後から MyOverlay でいつでも設定できます。",
        "بدون ذلك، لن يرفع MyOverlay مقاطع الفيديو إلى YouTube. كل شيء آخر يعمل بشكل طبيعي، ويمكنك إعداد ذلك لاحقًا من MyOverlay متى شئت.",
        "Sans cela, MyOverlay n'enverra pas de vidéos sur YouTube. Tout le reste fonctionne normalement, et vous pourrez le configurer plus tard depuis MyOverlay quand vous le souhaitez.",
        "Senza, MyOverlay non caricherà video su YouTube. Tutto il resto funziona normalmente e potrai configurarlo in seguito da MyOverlay quando vuoi.",
        "Без этого MyOverlay не будет загружать видео на YouTube. Всё остальное работает как обычно, и вы сможете настроить это позже в MyOverlay в любой момент.",
    ])),
...
ZH: dict[str, str] = {
    ...
    "GC_EXPLAIN": "Google Cloud SDK 随 MyOverlay 一起提供，继续时将完成设置。后续步骤中的 Google API 配置需要它。",
    "SC_TITLE": "快捷方式",
    "SC_DESC": "选择要创建的快捷方式。",
    ...
    "GA_WHAT": "MyOverlay 替您完成 Google 的设置：为 YouTube 上传准备好您的账号并保存所需的访问权限，您无需自己打开 Google Cloud Console。",
    "GA_WHY_H": "为什么需要它",
    "GA_WHY": "仅在将渲染后的视频上传到 YouTube 时才需要。其他所有功能——摄像头/遥测导入、MyChron 同步和叠加渲染——无需它即可运行。",
    "GA_NEXT_H": "您需要做的",
    "GA_NEXT": "首次运行 MyOverlay 时，使用拥有您 YouTube 频道的 Google 账号登录一次。这是您唯一需要做的操作。",
    "GA_SKIPNOTE": "如果您不想发布到 YouTube，可以跳过。",
    "SW_TITLE": "警告：将禁用 YouTube 发布",
    "SW_DESC": "您选择了跳过 Google API 配置。",
    "SW_TEXT": "不设置时，MyOverlay 不会将视频上传到 YouTube。其他功能均正常工作，您可以稍后随时在 MyOverlay 中进行设置。",
    "RES_TITLE": "默认输出分辨率",
