...
S: dict[str, dict[str, str]] = {
    ...
    "DEPS_TITLE": dict(zip(L, ["Software check", "Verificação de software", "Comprobación de software", "ソフトウェアの確認", "فحص البرامج", "Vérification des logiciels", "Verifica del software", "Проверка программ"])),
    "DEPS_DESC": dict(zip(L, ["Setup checked this computer for software MyOverlay works with.", "O instalador verificou neste computador os softwares com os quais o MyOverlay trabalha.", "El instalador comprobó en este equipo el software con el que trabaja MyOverlay.", "セットアップは、MyOverlay が連携するソフトウェアがこのコンピューターにあるかを確認しました。", "تحقق برنامج الإعداد من وجود البرامج التي يعمل معها MyOverlay على هذا الكمبيوتر.", "L'assistant a vérifié la présence sur cet ordinateur des logiciels avec lesquels MyOverlay fonctionne.", "Il programma di installazione ha verificato su questo computer il software con cui MyOverlay lavora.", "Программа установки проверила на этом компьютере наличие программ, с которыми работает MyOverlay."])),
    "DEPS_GROUP": dict(zip(L, ["Detected software", "Software detectado", "Software detectado", "検出されたソフトウェア", "البرامج المكتشفة", "Logiciels détectés", "Software rilevato", "Обнаруженные программы"])),
    "DEPS_FOUND": dict(zip(L, ["installed", "instalado", "instalado", "インストール済み", "مثبت", "installé", "installato", "установлено"])),
    "DEPS_MISSING": dict(zip(L, ["NOT FOUND", "NÃO ENCONTRADO", "NO ENCONTRADO", "見つかりません", "غير موجود", "INTROUVABLE", "NON TROVATO", "НЕ НАЙДЕНО"])),
    "DEPS_RS3_NOTE": dict(zip(L, [
        "Race Studio 3 is AiM's free program that downloads the telemetry from your MyChron - MyOverlay reads its data. Download it from www.aim-sportline.com (you can also install it later).",
        "O Race Studio 3 é o programa gratuito da AiM que baixa a telemetria do seu MyChron - o MyOverlay lê os dados dele. Baixe-o em www.aim-sportline.com (você também pode instalá-lo depois).",
        "Race Studio 3 es el programa gratuito de AiM que descarga la telemetría de su MyChron - MyOverlay lee sus datos. Descárguelo de www.aim-sportline.com (también puede instalarlo más tarde).",
        "Race Studio 3 は、MyChron からテレメトリをダウンロードする AiM の無料プログラムで、MyOverlay はそのデータを読み取ります。www.aim-sportline.com からダウンロードしてください（後からインストールしても構いません）。",
        "Race Studio 3 هو برنامج AiM المجاني الذي ينزل بيانات القياس عن بعد من جهاز MyChron - ويقرأ MyOverlay بياناته. نزله من www.aim-sportline.com (يمكنك أيضا تثبيته لاحقا).",
        "Race Studio 3 est le programme gratuit d'AiM qui télécharge la télémétrie de votre MyChron - MyOverlay lit ses données. Téléchargez-le sur www.aim-sportline.com (vous pouvez aussi l'installer plus tard).",
        "Race Studio 3 è il programma gratuito di AiM che scarica la telemetria dal tuo MyChron - MyOverlay ne legge i dati. Scaricalo da www.aim-sportline.com (puoi anche installarlo in seguito).",
        "Race Studio 3 - бесплатная программа AiM, которая скачивает телеметрию с вашего MyChron; MyOverlay читает её данные. Скачайте её с www.aim-sportline.com (можно установить и позже).",
    ])),
    "DEPS_WARN_NOTE": dict(zip(L, [
        "You can continue, but the features that depend on the missing software will not work until you install it.",
        "Você pode continuar, mas os recursos que dependem do software ausente não funcionarão até que ele seja instalado.",
        "Puede continuar, pero las funciones que dependen del software ausente no funcionarán hasta que lo instale.",
        "続行できますが、見つからないソフトウェアに依存する機能は、それをインストールするまで動作しません。",
        "يمكنك المتابعة، لكن الميزات التي تعتمد على البرنامج المفقود لن تعمل حتى تقوم بتثبيته.",
        "Vous pouvez continuer, mais les fonctionnalités qui dépendent du logiciel manquant ne fonctionneront pas tant qu'il ne sera pas installé.",
...
ZH: dict[str, str] = {
    ...
    "LANG_LABEL": "语言：",
    "DEPS_TITLE": "软件检查",
    "DEPS_DESC": "安装程序已检查此计算机上与 MyOverlay 配合使用的软件。",
    "DEPS_GROUP": "检测到的软件",
    "DEPS_FOUND": "已安装",
    "DEPS_MISSING": "未找到",
    "DEPS_RS3_NOTE": "Race Studio 3 是 AiM 的免费程序，用于从您的 MyChron 下载遥测数据——MyOverlay 读取它的数据。请从 www.aim-sportline.com 下载（也可以稍后安装）。",
